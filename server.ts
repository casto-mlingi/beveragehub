...
async function startServer() {
  ...
  app.get("/api/bulk-data", async (req, res) => {
    ...
    try {
      ...
      const basicEntities = [
        ...
        { key: 'quickActions', table: 'quick_actions' },
        { key: 'categories', table: 'categories' }
      ];

      // Fetch basic entities in parallel with individual error handling
      const basicPromises = basicEntities.map(async (entity) => {
        try {
          const queryRes = await pool.query(`SELECT * FROM ${entity.table}`);
          results[entity.key] = mapKeys(queryRes.rows);
        } catch (e) {
          console.error(`[BULK] Error fetching ${entity.key}:`, e);
          results[entity.key] = []; // Fallback to empty array
        }
      });

      // Special case: Fetch managers for everyone
      const managersPromise = (async () => {
        try {
          const queryRes = await pool.query("SELECT uid, email, name, role, phone, business_name, address, lat, lng FROM users WHERE role = 'manager'");
          results['managers'] = mapKeys(queryRes.rows);
        } catch (e) {
          console.error("[BULK] Error fetching managers:", e);
          results['managers'] = [];
        }
      })();
      ...
      if (isManager && companyId) {
        ...
        try {
          const res = await pool.query(`SELECT * FROM orders WHERE customer_name = $1`, [customerName]);
          results['orders'] = mapKeys(res.rows);
        } catch (e) {
          console.error("[BULK] Error fetching customer orders:", e);
          results['orders'] = [];
        }
      }
  ...
  app.get('/api/stream', (req, res) => {
    ...
    }, 15000); // More frequent heartbeat to keep connection alive through proxies
  ...
  entities.forEach(entity => {
    ...
    app.get(`/api/${entity}`, async (req, res) => {
      ...
      if (entity === 'users') {
        ...
        if (phone) return handleQuery(res, `SELECT * FROM ${table} WHERE phone = $1`, [phone]);
  ...
  app.all('/api/*', (req, res) => {
    const path = req.path;
    if (path === '/api/vehicles' || path === '/api/products' || path === '/api/ads') {
      console.warn(`[DEPRECATED] Old client requested: ${req.method} ${req.url}`);
    } else {
  ...
  // START LISTENING ONLY AFTER ALL ROUTES ARE REGISTERED
